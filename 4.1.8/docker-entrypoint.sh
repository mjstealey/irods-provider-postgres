...
_var_irods_tgz() {
  cp /var_irods.tar.gz /var/lib/irods/var_irods.tar.gz
  cd /var/lib/irods/
  echo "!!! populating /var/lib/irods with initial contents !!!"
  tar -zxvf var_irods.tar.gz
  cd /
  rm -f /var/lib/irods/var_irods.tar.gz
}

### populate contents of /etc/irods if external volume mount is used
_etc_irods_tgz() {
  cp /etc_irods.tar.gz /etc/irods/etc_irods.tar.gz
  cd /etc/irods/
  echo "!!! populating /etc/irods with initial contents !!!"
  tar -zxvf etc_irods.tar.gz
  cd /
  rm -f /etc/irods/etc_irods.tar.gz
}
...
if [[ ! -f /var/lib/irods/iRODS/irodsctl ]]; then
  _var_irods_tgz
fi
...
if [[ ! -d /var/lib/irods/.irods ]]; then
  ...
  _database_setup
  _generate_config
  sudo /var/lib/irods/packaging/setup_irods.sh < /irods.config
  _update_uid_gid
else
